...
test("external scanner fallback adapts to its observed key interval and stays below half a second", () => {
  ...
  assert.match(html, /mobile\.js\?v=(?:20260917-audit-partial-stock-v1|20260917-audit-eligible-status-v1|20260915-order-actions-menu-v1|20260914-shipping-status-sync-v1|20260910-product-image-track|20260911-inventory-move-persistence-v1)-(?:dev|prd)/);
});
...
test("external scanner keeps queue safety without exposing registration counters", () => {
  ...
  assert.doesNotMatch(source, /hardwareScannerResults|renderHardwareScannerProgress|`등록 \$\{|`중복 \$\{|`대기 \$\{/);
});
