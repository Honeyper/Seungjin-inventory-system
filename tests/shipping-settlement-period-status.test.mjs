...
test("출고 기간 상태 집계 변경본을 캐시 없이 불러온다", () => {
  assert.match(adminHtml, /admin\.js\?v=(?:20260911-(?:production-plan-(?:width|rows)|print-page-isolation|shipping-settlement-filters|inventory-audit-search|inventory-audit-client-colors|inventory-move-persistence|production-plan-delete|shipping-settlement-quantity|production-plan-no-label|production-plan-detail-toggle|production-capacity|production-inputs|remainder-save)-v1|20260910-(?:inventory-attachments|update-history(?:-v2)?|notification-scroll-v3|production-plan-tabs-v1|production-plan-product-v[23]|production-plan-detail-v[1234]))-(?:dev|prd)/);
});
