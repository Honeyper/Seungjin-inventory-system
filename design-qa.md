...
- 입고 첨부는 기준 화면의 2열 위치, 점선 테두리, 네이비 아이콘과 제목, 동일한 카드 크기를 유지했다. 후속 요청에 따라 카드 아래 보조 문구는 제거했다.
- 제품 이미지는 기준 화면의 미리보기 정사각형, 제목, 파일 안내, 선택·제거 버튼 구조를 그대로 유지하고 기존 안내 문구 안에 드롭 방법을 추가했다.
- 글꼴은 Noto Sans KR과 기존 굵기·위계를 유지했으며 카드 제목만 표시한다.
...
2. 후속 요청에서 드롭 보조 문구를 제거했으며, 드롭 이벤트와 접근성 이름으로 기능 안내는 유지했다.
...
## 77. DEV 입고 첨부 카드 보조 문구 제거

- 기준 화면: `/var/folders/01/8n9r8rnd7dv5hkhfy_7p521w0000gn/T/TemporaryItems/NSIRD_screencaptureui_oSg5X3/스크린샷 2026-09-04 오후 4.18.39.png`
- 구현 화면: `/private/tmp/seungjin-inbound-upload-no-helper-full.png`
- 동일 화면 비교: `/private/tmp/seungjin-inbound-upload-no-helper-comparison.png`
- 구현 뷰포트: 1280 × 720 CSS px, device scale factor 2
- 기준 이미지 크기: 678 × 196px
- 구현 전체 캡처 크기: 1280 × 810px. 비교 이미지는 입고 첨부 카드 영역을 원본 밀도로 잘라 기준 화면과 같은 캔버스에 배치했다.
- 비교 상태: 입고 등록의 빈 거래명세서·불량 사진 업로드 카드

### 전체 화면 및 집중 영역 비교

- 기준 화면에 표시된 작은 안내 문구 `클릭 또는 끌어다 놓기`, `여러 장 클릭 또는 끌어다 놓기`를 제거했다.
- 거래명세서·불량 사진 제목, 네이비 Tabler 아이콘, 2열 배치, 점선 테두리, 카드 높이와 기존 색상 토큰은 유지했다.
- 제품 이미지 영역과 실제 이미지 미리보기 동작에는 변경이 없다.
- 기준 화면은 확대된 카드 부분이고 구현 화면은 전체 관리자 폼이어서 절대 배율은 다르지만, 집중 영역에서 제목 아래 보조 문구가 사라지고 카드 구조가 유지된 것을 확인했다.

### 기능 및 접근성 확인

- DOM에서 `.upload-drop-hint` 요소가 0개임을 확인했다.
- 거래명세서, 불량 사진, 제품 이미지의 `data-file-drop-zone`과 각 입력 연결은 유지했다.
- 두 입고 첨부 버튼의 접근성 이름에는 `선택하거나 끌어다 놓기`가 남아 화면 문구 없이도 드롭 기능을 설명한다.
- 브라우저 콘솔 경고·오류: 없음.
- P0/P1/P2/P3: 없음.

### 비교 이력

1. 사용자 피드백에서 제목 아래 작은 보조 문구가 불필요한 요소로 확인됐다.
2. 보조 문구 요소와 전용 CSS만 제거하고 업로드 카드 및 드롭 이벤트 연결은 유지했다.
3. 수정 후 화면과 DOM을 다시 확인했으며 남은 시각적·기능적 회귀는 없다.
...
- 스타일 기준 화면: `/var/folders/01/8n9r8rnd7dv5hkhfy_7p521w0000gn/T/TemporaryItems/NSIRD_screencaptureui_JdULhS/스크린샷 2026-08-07 오후 12.20.07.png`
- 출고 확인 구현: `/private/tmp/seungjin-mobile-action-sheet-dev.png`
- 수량 변경 구현: `/private/tmp/seungjin-mobile-quantity-sheet-dev.png`
- 좁은 화면 수량 변경: `/private/tmp/seungjin-mobile-quantity-sheet-narrow.png`
- 기준 화면 픽셀: 896 × 998 px
- 구현 화면 픽셀: 각각 430 × 932 px
- 구현 뷰포트: 430 × 932 CSS px, device scale factor 1
- 비교 방식: 세 이미지를 한 번에 열어 현재 출고 화면의 디자인 언어와 두 팝업의 일관성을 비교했다. 기준 화면이 팝업 목업이 아니므로 원시 픽셀 복제가 아니라 제품 디자인 시스템의 일관성을 검증했다.

### 전체 화면 및 세부 비교

- 타이포그래피: Noto Sans KR과 미디움 500 두께를 유지했다. 시트 제목 21px, 제품명 15px, 보조문구 10~12px의 위계가 현재 모바일 카드와 일치한다.
- 간격과 레이아웃: 좌우 16px, 내부 간격 8~11px, 컴포넌트 반경 11~12px, 시트 상단 반경 22px, 안전영역을 포함한 고정 하단 버튼을 사용했다. 430 × 932에서 가로 넘침이나 버튼 잘림이 없다.
- 색상: 네이비 `#061f4c`, 골드 `#d39a22`, 흰색 패널, 청회색 경계선을 재사용했다. 출고대기·재고이동·삭제는 기본 팔레트 안에서만 의미색을 구분했다.
- 아이콘: 설치된 Tabler 아웃라인 아이콘을 사용했다. 새 인라인 SVG, 이모지, CSS 드로잉 또는 대체 플레이스홀더는 추가하지 않았다.
- 문구: `출고`, `출고대기`, `박스 수량 변경`, `재고 수정`, `박스 전량 이동` 용어를 유지했고 목록 삭제는 실제 재고가 바뀌지 않는다는 설명을 추가했다.
- 집중 비교: 헤더의 작은 분류명·제목·42px 아이콘, 골드 좌측선이 있는 제품 요약, 50px 취소/확인 버튼, 수량 변경의 2열 박스 선택과 숫자 스테퍼를 별도로 확인했다.

### 기능 및 접근성 확인

- 출고 확인, 출고대기, 출고/대기 취소, 재고 이동, 사출재고 등록, 출고·재고수정 목록 삭제를 동일한 바텀시트로 통합했다.
- 기존 닫기·취소·확인·수량변경·박스선택 ID와 이벤트 연결을 유지했다.
- 팝업이 열리면 닫기 버튼으로 초점을 이동하고, 닫을 때 기존 버튼으로 초점을 되돌리며, `Escape` 닫기를 지원한다.
- 모바일 흐름의 `window.confirm`, `window.prompt`, `window.alert` 호출을 제거했다.
- 브라우저 렌더링과 DOM 구조를 확인했고 콘솔 경고·오류는 없었다.
- 360 × 800 좁은 뷰포트에서도 수량 입력과 취소·수량 변경 버튼이 잘리지 않았다.
- 실제 출고 및 재고 변경 요청은 실데이터를 바꾸는 작업이므로 디자인 QA에서 실행하지 않았다.

### 비교 이력 및 결과

- 1차 비교에서 출고 확인과 수량 변경 시트 모두 현재 모바일 화면의 팔레트, 글꼴, 간격, 아이콘 굵기, 버튼 위계와 일치했다.
- P0: 없음
- P1: 없음
- P2: 없음
- P3: 현장 구형 갤럭시에서 키보드가 열린 수량 변경 화면의 높이를 한 번 더 확인할 수 있다.

final result: passed

## 72. DEV QR 라벨 기준 이미지 반영

### 비교 환경

- 기준 이미지: `/var/folders/01/8n9r8rnd7dv5hkhfy_7p521w0000gn/T/codex-clipboard-1a92bbc6-a596-4b35-9d24-85a48a467197.png`
- 브라우저 구현 캡처: `/private/tmp/seungjin-qr-implementation-final.png`
- 기준 이미지: 2240 × 1912 px, 비교용 793 × 677 px로 정규화
- 구현 이미지: 793 × 673 px
- 라벨 크기: 70mm × 59.4mm, 비교 캡처 3배 확대, 브라우저 DPR 2
- 상태: 최종공정 1도, 화염처리 있음, 54박스, 5차 입고
- 브라우저 콘솔 경고·오류: 없음

### 전체 화면 및 세부 비교

- 제목, 기준수량·입고일, 제품명·QR, 5열 공정표, 3개 공정 행, 화면의 검은 비활성 행, 관리자 행의 비율과 테두리를 기준 이미지에 맞췄다.
- 글꼴은 한글 시스템 폴백을 사용하고 인쇄 크기에서 위계·줄바꿈·굵기를 기준 이미지와 가깝게 조정했다.
- 21/23/19/18/19 열 비율과 행 높이, 화면의 검정·흰색 배색, 둥근 외곽선을 동일하게 유지했다.
- 실제 QR 이미지를 비율 변경 없이 가운데 배치했다.
- 기준 이미지의 `화염` 표기는 사용자 지정 용어인 `화염처리`로 의도적으로 변경했다.
- 793px 전체 라벨에서 모든 글자와 경계선을 판독할 수 있어 별도 집중 크롭은 필요하지 않았다.

### 비교 이력 및 결과

1. 1차 비교에서 테두리와 글자 두께가 기준보다 무겁고 제품 영역 좌우 여백과 QR 크기가 큰 P2 차이를 확인했다.
2. 테두리·모서리·글자 두께, 제품 여백과 QR 크기를 보정했다.
3. 재캡처 비교에서 남은 P0·P1·P2 차이가 없고 콘솔 경고·오류도 없음을 확인했다.

- P0: 없음
- P1: 없음
- P2: 없음
- P3: 프린터 및 운영체제의 한글 폰트에 따라 글자 모양에 미세한 차이가 날 수 있다.

final result: passed

## 73. DEV QR 비활성 공정 인쇄 색상 복구

- 화면에서는 기준 이미지처럼 비활성 공정을 검은 행으로 유지한다.
...
## 75. 모바일 출고 스캐너 하단 작업 버튼 고정

- 기준 화면: `/Users/kang-kyoungmo/Downloads/KakaoTalk_Photo_2026-09-02-09-56-19 004.jpeg`
- 구현 화면: `/private/tmp/mobile-scanner-actions-fixed.png`
- 기준 이미지: 1248 × 1972px, 휴대폰 세로 화면
- 구현 이미지: 412 × 680px, CSS 뷰포트 412 × 680px, 캡처 DPR 2
- 상태: 출고 QR 3건 스캔, 성공 알림 노출, 기본 시트 상태
- 전체 비교: 기준 화면에서는 3건 스캔 시 하단 `출고대기 등록`·`출고` 버튼이 화면 아래로 밀렸고, 구현 화면에서는 같은 3건 상태에서 두 버튼이 화면 하단에 고정되어 모두 보인다.
- 집중 비교: 스캔 목록 하단과 작업 버튼 경계를 별도로 확인했다. 목록 영역 하단 620px, 버튼 상단 628px, 버튼 하단 672px로 겹침이나 화면 잘림이 없다.
- 글꼴·색상·아이콘·문구: 기존 모바일 네이비·골드 팔레트, Noto Sans KR, 기존 버튼과 스캔 카드 구조를 변경하지 않았다.
- 레이아웃: 숨김 처리되는 재고 작업 선택 행과 무관하게 핸들·헤더·선택 행·목록·작업 버튼을 각각 고정된 그리드 행에 배치했다. 목록만 세로 스크롤되고 작업 버튼은 항상 마지막 행에 남는다.
- 이미지 품질: 카메라 영상과 기존 이미지 자산 처리에는 변경이 없다.
- 브라우저 콘솔 경고·오류: 없음

### 비교 이력 및 결과

1. 1차 구현 확인에서 숨김 행 때문에 목록과 작업 버튼이 같은 그리드 영역을 침범하는 P1 문제를 확인했다.
2. 각 영역에 명시적인 그리드 행 번호를 부여하고 목록만 스크롤되도록 수정했다.
3. 재검증에서 작업 버튼이 화면과 패널 안에 완전히 표시되고 목록과 겹치지 않음을 확인했다.
