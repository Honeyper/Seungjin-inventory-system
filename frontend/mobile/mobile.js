...
const INVENTORY_MOVE_SCAN_ACTIONS = {
  move: { label: "자리이동", target: "이동 위치" },
...
const elements = {
  ...
  inventoryMoveActionPicker: document.querySelector("#inventoryMoveActionPicker"),
...
function renderInventoryMoveItem(item) {
  ...
  const boxScope = getInventoryMoveBoxScope(item);
  const managementBoxOverview = buildInventoryMoveBoxOverview(item, "management");
  const productBoxOverview = buildInventoryMoveBoxOverview(item, "product");
  const boxOverview = boxScope === "product" ? productBoxOverview : managementBoxOverview;
  ...
  return `
    <article class="shipping-item shipping-product-card inventory-move-item ${getClientToneClass(item.clientName)}" data-inventory-move-item="${escapeHtml(key)}">
        ...
        <div class="shipping-card-status-actions">
          ...
          <details class="shipping-card-menu">
            ...
            <div class="shipping-card-menu-popover" role="menu" aria-label="재고 수정 작업">
              ...
              <button type="button" role="menuitem" data-inventory-move-action="audit" data-inventory-move-key="${escapeHtml(key)}"><i class="ti ti-clipboard-check"></i>재고 실물 확인</button>
      ...
      ${renderInventoryMoveBoxOverview({
        key,
        scope: boxScope,
        overview: boxOverview,
        managementBoxCount: managementBoxOverview.boxCount,
        productBoxCount: productBoxOverview.boxCount
      })}
...
function handleInventoryMoveListClick(event) {
  ...
  if (scopeButton) {
    ...
    if (!item) {
      showToast("박스 현황을 확인할 제품을 찾지 못했습니다.");
      return;
    }
    state.inventoryMoveBoxScopes[key] = normalizeInventoryMoveBoxScope(scopeButton.dataset.inventoryBoxScope);
    renderInventoryMoveList();
    return;
  }
...
function getReturnableShippingBoxes(item, returnMode = "transfer") {
  ...
  sourceItems.forEach((sourceItem) => {
    const shippedBoxes = Array.isArray(sourceItem?.shippedShippingBoxes)
      ? sourceItem.shippedShippingBoxes
      : [];
    ...
    candidates.forEach((box) => {
      ...
      const number = parseNumber(box?.number || box?.sequence || sourceItem?.scannedBoxNumber);
      if (number <= 0) {
        return;
      }

      const managementId = normalizeScanValue(sourceItem?.managementId || item?.managementId);
      const key = `${managementId}__${number}`;
      if (seen.has(key)) {
        return;
      }
      seen.add(key);

      const shippingType = String(box?.shippingType || box?.["출고유형"] || "").trim();
      const companyMatch = shippingType.match(/^이관\s*\(([^)]+)\)/);
      results.push({
        key,
        number,
        quantity: parseNumber(box?.currentQuantity || box?.quantity || box?.boxQuantity || sourceItem?.boxQuantity),
        shippingType,
        transferCompany: box?.transferCompany || companyMatch?.[1] || "",
        sourceItem,
        managementId,
        productId: sourceItem?.productId || item?.productId || "",
        productName: sourceItem?.productName || item?.productName || "",
        clientName: sourceItem?.clientName || item?.clientName || "",
        batch: sourceItem?.batch || item?.batch || "",
        finalProcess: sourceItem?.finalProcess || item?.finalProcess || ""
      });
    });
  });
...
function renderMobileTransferReturnStorageOptions() {
  ...
  elements.mobileTransferReturnStorage.innerHTML = [
    '<option value="">선택하세요.</option>',
    ...INVENTORY_STORAGE_OPTIONS.map((storage) => `<option value="${escapeHtml(storage)}">${escapeHtml(storage)}</option>`)
  ].join("");
}
...
function renderMobileTransferReturnControls(item, returnMode = "transfer") {
  ...
  if (elements.mobileTransferReturnBoxList) {
    elements.mobileTransferReturnBoxList.innerHTML = boxes.map((box, index) => `
      <label>
        <input type="checkbox" name="mobileTransferReturnBox" value="${index}" checked />
        <span><strong>${formatNumber(box.number)}번 박스</strong><small>${formatNumber(box.quantity)} ea${box.transferCompany ? ` · ${escapeHtml(box.transferCompany)}` : ""}</small></span>
      </label>
    `).join("");
  }

  if (elements.mobileTransferReturnStorage) {
    const currentStorage = normalizeDisplay(item?.storage || "");
    elements.mobileTransferReturnStorage.value = INVENTORY_STORAGE_OPTIONS.includes(currentStorage)
      ? currentStorage
      : "";
  }
  document.querySelector('input[name="mobileTransferReturnStatus"][value="보관"]')?.click();
  if (elements.mobileTransferReturnMessage) {
    elements.mobileTransferReturnMessage.textContent = "";
  }
  syncMobileTransferReturnSummary();
}

function getSelectedMobileTransferReturnBoxes() {
  return Array.from(elements.mobileTransferReturnBoxList?.querySelectorAll('input[name="mobileTransferReturnBox"]:checked') || [])
    .map((input) => state.activeTransferReturnBoxes[Number(input.value)])
    .filter(Boolean);
}

function syncMobileTransferReturnSummary() {
  const selected = getSelectedMobileTransferReturnBoxes();
  const quantity = selected.reduce((sum, box) => sum + box.quantity, 0);
  if (elements.mobileTransferReturnBoxSummary) {
    elements.mobileTransferReturnBoxSummary.textContent = selected.length
      ? `${formatNumber(selected.length)}박스 · ${formatNumber(quantity)} ea`
      : "박스를 선택해주세요.";
  }
}
...
function closeConfirmModal() {
  ...
  if (elements.mobileTransferReturnBoxList) {
    elements.mobileTransferReturnBoxList.innerHTML = "";
  }
  if (elements.mobileTransferReturnMessage) {
    elements.mobileTransferReturnMessage.textContent = "";
  }
...
async function handleReturnTransferredInventory(item) {
  ...
  selectedBoxes.forEach((box) => {
    if (!groups.has(box.managementId)) {
      groups.set(box.managementId, {
        ...box,
        selectedBoxes: []
      });
    }
    groups.get(box.managementId).selectedBoxes.push(box.number);
  });
    ...
    const results = await mapWithConcurrency(Array.from(groups.values()), SHIPPING_ACTION_CONCURRENCY, async (group) => {
      try {
        const response = await requestApi(isTakeoutReturn ? "returnTakenOutInventory" : "returnTransferredInventory", {
          ...
          batch: group.batch,
          finalProcess: group.finalProcess,
          selectedBoxes: group.selectedBoxes,
          storage,
          targetStatus,
          returner: state.user?.name || "Admin"
        });
        return {
          count: Array.isArray(response?.returnedBoxes) ? response.returnedBoxes.length : group.selectedBoxes.length,
          error: ""
        };
      } catch (error) {
...
function handleScannerDoneAction() {
  if (state.activeWorkflow === "inventoryMove") {
    ...
    if (action === "audit") {
      handleScannerInventoryAuditAction();
      return;
    }
    openScannedInventoryMoveConfirmModal(action === "move" ? "all" : action);
    return;
  }
...
function normalizeInventoryMoveBoxScope(value) {
  return value === "product" ? "product" : "management";
}

function getInventoryMoveBoxScope(item) {
  return normalizeInventoryMoveBoxScope(
    state.inventoryMoveBoxScopes[getInventoryMoveKey(item)]
  );
}

function buildInventoryMoveBoxOverview(anchorItem, scope = "management") {
  const normalizedScope = normalizeInventoryMoveBoxScope(scope);
  const anchorManagementId = normalizeScanValue(anchorItem?.managementId);
  const anchorProductKey = getInventoryAuditProductKey(anchorItem);
  const scannedBoxKeys = new Set(
    state.scannedMoveRows.map((row) => getInventoryAuditBoxKey(row)).filter(Boolean)
  );
  const seenBoxKeys = new Set();
  const boxes = [];

  state.dashboard.forEach((row) => {
    const isInScope = normalizedScope === "product"
      ? getInventoryAuditProductKey(row) === anchorProductKey
      : normalizeScanValue(row?.managementId) === anchorManagementId;
    if (!isInScope) {
      return;
    }
    ...
    getMovableBoxes(row).forEach((box) => {
      ...
      if (!boxKey || seenBoxKeys.has(boxKey) || isProtectedInventoryAdjustmentBox(row, box)) {
        return;
      }
      ...
      boxes.push({
        boxKey,
        boxNumber: String(box?.number || box?.sequence || "").trim(),
        quantity: getBoxCurrentQuantity(box, row),
        storage: normalizeDisplay(box?.storage || row?.storage || "미지정"),
        inboundDate: formatManualShippingInboundDate(row?.inboundDate),
        managementId: normalizeDisplay(row?.managementId || "-"),
        isScanned: scannedBoxKeys.has(boxKey)
      });
    });
  });

  boxes.sort((left, right) => {
    const dateOrder = String(right.inboundDate).localeCompare(String(left.inboundDate), "ko", { numeric: true });
    if (dateOrder !== 0) {
      return dateOrder;
    }
    const managementOrder = left.managementId.localeCompare(right.managementId, "ko", { numeric: true });
    if (managementOrder !== 0) {
      return managementOrder;
    }
    return parseNumber(left.boxNumber) - parseNumber(right.boxNumber);
  });
  ...
  return {
    ...
    quantity: boxes.reduce((sum, box) => sum + box.quantity, 0),
    scannedBoxCount: boxes.filter((box) => box.isScanned).length
  };
}

function renderInventoryMoveBoxOverview({
  key,
  scope,
  overview,
  managementBoxCount,
  ...
  const scopeLabel = scope === "product" ? "같은 SKU 전체" : "해당 입고 건";
  const boxCards = overview.boxes.length
    ? overview.boxes.map((box) => `
      <article class="inventory-remaining-box ${box.isScanned ? "is-scanned" : ""}">
        <span class="inventory-remaining-box-head">
          <strong>${escapeHtml(normalizeDisplay(box.boxNumber))}번 박스</strong>
          <em>${box.isScanned ? "QR 확인" : "미확인"}</em>
        </span>
        <b>${formatNumber(box.quantity)} <small>ea</small></b>
        <span class="inventory-remaining-box-meta">입고 ${escapeHtml(box.inboundDate)} · ${escapeHtml(box.storage)}</span>
        <small class="inventory-remaining-box-management">${escapeHtml(box.managementId)}</small>
      </article>
    `).join("")
    : '<p class="inventory-remaining-box-empty">선택한 범위에 보관 중인 일반재고 박스가 없습니다.</p>';

  return `
    <section class="inventory-box-overview" aria-label="${escapeHtml(scopeLabel)} 남은 박스 현황">
      <div class="inventory-box-overview-head">
        <span>
          <small>남은 박스 현황</small>
          <strong>${formatNumber(overview.boxCount)}박스 · ${formatNumber(overview.quantity)}ea</strong>
        </span>
        <em>QR 확인 ${formatNumber(overview.scannedBoxCount)}</em>
      </div>
      <div class="inventory-box-scope-switch" role="group" aria-label="남은 박스 조회 범위">
        <button type="button" data-inventory-box-scope="management" data-inventory-move-key="${escapeHtml(key)}" aria-pressed="${scope === "management"}">
          <span>해당 입고 건</span>
          <small>${formatNumber(managementBoxCount)}박스</small>
        </button>
        <button type="button" data-inventory-box-scope="product" data-inventory-move-key="${escapeHtml(key)}" aria-pressed="${scope === "product"}">
          <span>같은 SKU 전체</span>
          <small>${formatNumber(productBoxCount)}박스</small>
        </button>
      </div>
      <div class="inventory-remaining-box-list">${boxCards}</div>
      <p class="inventory-box-overview-guide">QR로 확인한 박스와 아직 확인하지 않은 박스를 구분해 보여드립니다.</p>
    </section>
  `;
}
...
function buildMissingInventoryAdjustmentPlan({ scope = "management", selectedItem = null } = {}) {
  ...
  scannedRows.forEach((row) => {
    ...
    const boxNumber = Number(box?.number || box?.sequence);
    const managementId = String(row?.managementId || "").trim();
    if (!managementId || !Number.isFinite(boxNumber) || boxNumber <= 0) {
      invalidBoxCount += 1;
      return;
    }

    seenConfirmedBoxKeys.add(boxKey);
    const productKey = getInventoryAuditProductKey(row);
    const groupKey = `${managementId}|${productKey}`;
    if (!confirmationGroups.has(groupKey)) {
      confirmationGroups.set(groupKey, {
        managementId,
        productId: row.productId || "",
        clientName: row.clientName || "",
        productName: row.productName || "",
    ...
    confirmationGroups.get(groupKey).selectedBoxes.push(boxNumber);
  });

  const confirmedBoxes = Array.from(confirmationGroups.values()).map((group) => ({
    ...group,
    selectedBoxes: group.selectedBoxes.slice().sort((left, right) => left - right)
  }));
...
function renderInventoryAuditScopeOptions(anchorItem, preparedPlans = null) {
  ...
  const availablePlans = plans.filter(({ plan }) => plan.confirmedBoxCount > 0 && plan.invalidBoxCount === 0);
...
async function completeMissingInventoryAdjustment(plan) {
  if (state.isCompletingShipping || !plan?.confirmedBoxes?.length) {
    return;
  }
  ...
  try {
    ...
    showToast(error.message || "재고 실물 확인 중 문제가 발생했습니다.");
  } finally {
    state.isCompletingShipping = false;
    updateScannerActionLabels();
  }
...
function isInventoryShippingScanAction(action) {
...
async function handleCompleteScannedInventoryShipping(mode) {
  if (state.isCompletingShipping || !isInventoryShippingScanAction(mode)) {
    return;
  }

  const items = groupScannedInventoryMoveRows(state.scannedMoveRows);
  const actionLabel = INVENTORY_MOVE_SCAN_ACTIONS[mode].label;
  if (!state.scannedMoveRows.length) {
    showToast(`${actionLabel} 처리할 박스를 다시 확인해주세요.`);
    return;
  }
  ...
  updateScannerActionLabels();
  try {
    const { completedCount, failedItems, errors } = await completeInventoryShippingItems(items, mode);
    if (completedCount > 0) {
      triggerScanFeedback(SCAN_COMPLETE_VIBRATION);
      ...
      showToast(errors[0] || `${actionLabel} 처리된 박스가 없습니다.`);
    }
  } catch (error) {
    showToast(error.message || `${actionLabel} 처리 중 문제가 발생했습니다.`);
  } finally {
    state.isCompletingShipping = false;
    updateScannerActionLabels();
  }
...
async function completeInventoryShippingItems(items, mode) {
  let completedCount = 0;
  const failedItems = [];
  const errors = [];
  ...
  for (const item of items) {
    ...
    try {
      const result = await completeInventoryShippingItem(item, selectedBoxes, mode);
      applyInventoryShippingResultLocally(item, selectedBoxes, INVENTORY_MOVE_SCAN_ACTIONS[mode].label);
      const updatedCount = parseNumber(result?.updatedBoxRows);
      completedCount += updatedCount > 0 ? updatedCount : selectedBoxes.length;
    } catch (error) {
      failedItems.push(item);
      errors.push(error?.message || `${INVENTORY_MOVE_SCAN_ACTIONS[mode].label} 처리 중 문제가 발생했습니다.`);
    }
  }
...
function completeInventoryShippingItem(item, selectedBoxes, mode) {
  const now = new Date();
  const shippingType = INVENTORY_MOVE_SCAN_ACTIONS[mode].label;
  return requestApi("updateShippingStatus", {
    managementId: item.managementId,
    productId: item.productId,
    clientName: item.clientName,
    ...
    batch: item.batch,
    finalProcess: item.finalProcess,
    storageLocation: getInventoryMoveCurrentStorage(item),
    ...
    boxQuantities: getSelectedBoxQuantities(item, selectedBoxes),
    allowInventoryAdjustment: true
  });
}

function applyInventoryShippingResultLocally(item, selectedBoxes, shippingType) {
  const managementId = normalizeScanValue(item?.managementId);
  const productId = normalizeScanValue(item?.productId);
...
function openInventoryMoveScanner() {
  state.activeWorkflow = "inventoryMove";
  state.selectedInventoryMoveAction = "move";
  openScanner();
}
...
function handleInventoryMoveActionChange(event) {
  ...
  state.selectedInventoryMoveAction = action;
  renderScannerScannedList();
  updateScannerActionLabels();
  const actionLabel = INVENTORY_MOVE_SCAN_ACTIONS[action].label;
  setScannerHelp(`${actionLabel} 처리할 박스 QR을 스캔해주세요.`);
}
...
async function setScannerInputMode(mode, options = {}) {
  ...
  if (nextMode === "hardware") {
    ...
    setScannerHelp(state.activeWorkflow === "inventoryMove"
      ? `외부 스캐너로 ${INVENTORY_MOVE_SCAN_ACTIONS[getInventoryMoveScanAction()].label} 처리할 박스 QR을 읽어주세요.`
      : "외부 스캐너로 제품 박스 QR을 읽으면 자동으로 등록됩니다.");
    return;
  }

  setScannerHelp(state.activeWorkflow === "inventoryMove"
    ? `${INVENTORY_MOVE_SCAN_ACTIONS[getInventoryMoveScanAction()].label} 처리할 박스 QR을 카메라에 비춰주세요.`
...
function syncScannerTorchControl() {
  ...
  // Android browsers do not consistently expose getCapabilities().torch even
  // when the rear camera accepts a torch constraint. A live track with
  // applyConstraints is therefore allowed one real capability probe on tap.
  state.scannerTorchSupported = Boolean(track?.applyConstraints) && track.readyState === "live";
...
async function toggleScannerTorch() {
  ...
  if (!state.scannerTorchSupported) {
    showToast("현재 카메라 또는 브라우저에서는 플래시를 사용할 수 없습니다.");
    return;
  }
  ...
  try {
    ...
    if (state.scannerTorchTrack === track) {
      showToast("이 카메라에서는 플래시를 켤 수 없습니다. 기본 카메라로 다시 열어주세요.");
    }
  } finally {
    if (state.scannerTorchTrack === track) {
      state.scannerTorchPending = false;
...
function updateScannerActionLabels() {
  ...
  if (state.activeWorkflow === "inventoryMove") {
    ...
    if (elements.inventoryMoveActionPicker) {
    ...
    elements.scannerDoneButton.hidden = false;
    elements.scannerInjectionButton.hidden = true;
    elements.scannerInventoryAuditButton.hidden = true;
    scannerBottom?.classList.toggle("single-action", !isMoveAction);
  ...
  if (elements.inventoryMoveActionPicker) {
    elements.inventoryMoveActionPicker.hidden = true;
  }
...
async function applyScannerTrackControls(track, controls) {
  ...
  const pending = previous.catch(() => {}).then(async () => {
    ...
    const preservedBase = retainUnchanged(base);
    const attempts = torch === undefined ? [{
      ...preservedBase,
      advanced: [...persistentControls, ...nonTorchUpdates]
    }] : [{
      ...preservedBase,
      advanced: [...persistentControls, ...nonTorchUpdates, { torch }]
    }, {
      ...preservedBase,
      torch,
      advanced: [...persistentControls, ...nonTorchUpdates]
...
async function submitHardwareScannerValue(rawValue) {
  ...
  queueHardwareScannerValue(value);
}

function queueHardwareScannerValue(value) {
  clearHardwareScannerStatusTimer();
...
async function handleQrValue(rawValue) {
  ...
  try {
    ...
    if (!matched) {
      const inventoryActionLabel = INVENTORY_MOVE_SCAN_ACTIONS[getInventoryMoveScanAction()].label;
      setScannerHelp(state.activeWorkflow === "inventoryMove" ? `${inventoryActionLabel} 처리할 박스를 찾지 못했습니다. QR 또는 보관 상태를 확인해주세요.` : "일치하는 박스를 찾지 못했습니다. QR 또는 박스 정보를 확인해주세요.");
      showToast(state.activeWorkflow === "inventoryMove" ? `${inventoryActionLabel} 처리할 박스가 없습니다.` : "일치하는 박스가 없습니다.");
      return;
    }
...
function renderScannerScannedList() {
  ...
  if (!rows.length) {
    const emptyInstruction = isInventoryMove
      ? state.scannerInputMode === "hardware"
        ? `외부 스캐너로 ${INVENTORY_MOVE_SCAN_ACTIONS[getInventoryMoveScanAction()].label} 처리할 박스 QR을 읽어주세요.`
        : `${INVENTORY_MOVE_SCAN_ACTIONS[getInventoryMoveScanAction()].label} 처리할 박스 QR을 카메라에 맞춰주세요.`
      : state.scannerInputMode === "hardware"
  ...
  let inventoryBoxOverview = "";
  if (isInventoryMove) {
    const latestGroup = groupScannedInventoryMoveRows(rows)[0];
    const key = getInventoryMoveKey(latestGroup);
    const scope = getInventoryMoveBoxScope(latestGroup);
    const managementOverview = buildInventoryMoveBoxOverview(latestGroup, "management");
    const productOverview = buildInventoryMoveBoxOverview(latestGroup, "product");
    inventoryBoxOverview = `
      <div class="scanner-inventory-box-overview">
        <p>방금 스캔한 제품의 재고를 확인해주세요.</p>
        ${renderInventoryMoveBoxOverview({
          key,
          scope,
          overview: scope === "product" ? productOverview : managementOverview,
          managementBoxCount: managementOverview.boxCount,
          productBoxCount: productOverview.boxCount
        })}
      </div>
    `;
  }

  elements.scannerScannedList.innerHTML = inventoryBoxOverview + rows.map((item, index) => {
    const scannedBox = getScannedBox(item);
